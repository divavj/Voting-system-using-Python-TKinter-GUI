...
def about():
    messagebox.showinfo('About','Developed by Diva')


home=Tk()
home.geometry('450x400')
home.title('Voting Program')
home['bg'] = '#A533FF'
Label(home,text='TamilNadu state election',font='Helvetica 24 bold',bg='#49A',justify='center').grid(row=1,column=4)
Button(home,text='Create new Poll +',command=create).grid(row=5,column=4)
Button(home,text='My Polls',command=polls).grid(row=4,column=4)
Button(home,text='Poll Results',command=selpl).grid(row=8,column=4)
Button(home,text='About',command=about).grid(row=1,column=6)
home.mainloop()